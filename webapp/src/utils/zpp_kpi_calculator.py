...
def fetch_duplicate_orders() -> Dict:
    """
    Busca registros com campo `ordem` duplicado nas collections ZPP_Producao e ZPP_Paradas.
    Não aplica filtro de data — varredura completa das collections.

    Returns:
        {
            "producao": list[dict],   # registros duplicados de ZPP_Producao
            "paradas":  list[dict],   # registros duplicados de ZPP_Paradas
        }
        Cada registro inclui "qtd_duplicatas" indicando quantas vezes aquela ordem aparece.
    """
    def _fmt_date(v):
        if v is None:
            return None
    ...
    result = {"producao": [], "paradas": []}

    # ── ZPP_Producao ──────────────────────────────────────────────────
    try:
        col_prod = get_mongo_connection(ZPP_PRODUCAO_COLLECTION)

        # Encontrar ordens que aparecem mais de uma vez
        dup_ordens_prod = {
            doc["_id"]
            for doc in col_prod.aggregate([
                {"$match": {"ordem": {"$exists": True, "$ne": None}}},
        ...
        if dup_ordens_prod:
            cursor = col_prod.find(
                {"ordem": {"$in": list(dup_ordens_prod)}},
                {"ordem": 1, "pto_trab": 1, "fininotif": 1, "ffinnotif": 1,
                 "horasact": 1, "_processed": 1, "_id": 0},
            ).sort("ordem", 1)

            # Contar ocorrências por ordem para exibir
            from collections import Counter
            all_prod = list(cursor)
            count_map = Counter(r.get("ordem") for r in all_prod)

            result["producao"] = [
                {
                    "ordem":          r.get("ordem"),
                    "qtd_duplicatas": count_map[r.get("ordem")],
                    "pto_trab":       r.get("pto_trab"),
                    "fininotif":      _fmt_date(r.get("fininotif")),
                    "ffinnotif":      _fmt_date(r.get("ffinnotif")),
                    "horasact":       r.get("horasact"),
                    "_processed":     r.get("_processed", False),
                }
                for r in all_prod
            ]
    except Exception as e:
        logger.error("Erro ao buscar duplicatas em %s: %s", ZPP_PRODUCAO_COLLECTION, e)

    # ── ZPP_Paradas ───────────────────────────────────────────────────
    try:
        col_par = get_mongo_connection(ZPP_PARADAS_COLLECTION)

        dup_ordens_par = {
            doc["_id"]
            for doc in col_par.aggregate([
                {"$match": {"ordem": {"$exists": True, "$ne": None}}},
                {"$group": {"_id": "$ordem", "count": {"$sum": 1}}},
                {"$match": {"count": {"$gt": 1}}},
            ])
        ...
        if dup_ordens_par:
            cursor = col_par.find(
                {"ordem": {"$in": list(dup_ordens_par)}},
                {"ordem": 1, "centro_de_trabalho": 1, "inicio_execucao": 1,
                 "fim_execucao": 1, "causa_do_desvio": 1, "duration_min": 1,
                 "_processed": 1, "_id": 0},
            ).sort("ordem", 1)

            from collections import Counter
            all_par = list(cursor)
            count_map = Counter(r.get("ordem") for r in all_par)

            result["paradas"] = [
                {
                    "ordem":               r.get("ordem"),
                    "qtd_duplicatas":      count_map[r.get("ordem")],
                    "centro_de_trabalho":  r.get("centro_de_trabalho"),
                    "inicio_execucao":     _fmt_date(r.get("inicio_execucao")),
                    "fim_execucao":        _fmt_date(r.get("fim_execucao")),
                    "causa_do_desvio":     r.get("causa_do_desvio"),
        ...
        logger.error("Erro ao buscar duplicatas em %s: %s", ZPP_PARADAS_COLLECTION, e)
