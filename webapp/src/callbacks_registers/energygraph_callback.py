...
def register_energygraph_callbacks(app, collection_energia):
    ...
    @app.callback(
        Output('stored-energy-data', 'data'), # <-- NOVO Output para o novo Store
        [
    ...
    def fetch_energy_data(n_intervals, start_date, end_date, start_hour, end_hour):
        ...
        try:
            ...
            # Busca na coleção específica de energia
    ...
    # Ele lê os dados do seu próprio Store: 'stored-energy-data'
    @app.callback(
        [Output('energy-graph', 'figure'),
         Output('energy-graph', 'style')],
        [Input('stored-energy-data', 'data'), # <-- Lê do seu próprio Store
         Input('url', 'pathname')],
